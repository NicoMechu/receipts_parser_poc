"""CLI entrypoint for OCR + optional Ollama CSV export."""
...
@click.option("--lang", default="es", show_default=True, help="PaddleOCR language code.")
@click.option(
    "--use-angle-cls",
    is_flag=True,
    default=False,
...
def main(
    ...
    """CLI entry: configure logging, validate input, run OCR, and write outputs."""
...
def run_parsing(
    ...
    *,
    ...
    for image_path in image_files:
        ...
        logger.debug("Plain text length for %s: %d chars", image_path.name, len(plain_text))
        ...
        if use_ollama:
            ...
            try:
                ...
                logger.warning("Ollama CSV step failed for %s: %s", image_path.name, exc)

    json_path.write_text(json.dumps(all_results, indent=2), encoding="utf-8")
    logger.info("Saved JSON output to: %s", json_path)
    logger.info("Done: processed %d image file(s) under %s", len(image_files), input_folder)
